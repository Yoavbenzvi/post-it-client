...
const Post = ({
	...
	const handleClick = () => {
		baseURL.get(`/get-user-id/${email}`).then((res) => {
	...
	return (
		<div className="text-s shadow-lg my-5 py-3 px-6 rounded-md bg-white">
			<div className="flex items-end pb-1">
				...
				<div className="font-bold text-xs bold mx-3">{name}</div>
			...
			<div>
				...
				<div className="flex">
					...
					<div className="text-center h-4 w-4">{likes.length}</div>
				</div>
			</div>
		</div>
	);
